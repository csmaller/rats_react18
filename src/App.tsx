...
const App: React.FC = () => {
  ...
  const tour = React.useRef(null);
    ...
    const scrollIntoView = (elref: React.RefObject<HTMLElement>) => {
     ...
     if(main && elref.current){
      ...
      main.scrollTo({ behavior: 'smooth' , top:elref.current?.getBoundingClientRect().top + main.scrollTop -120 });//<--uncomment to enable scrolling to ref
  ...
  return (
    <Container id="main" sx={{overflowY:"scroll", height:"100vh", width:"100%" , maxWidth:"100% !important", scrollBehavior:"smooth", scrollSnapType:"y mandatory", '& > *': { scrollSnapAlign: 'start' }}}>
       <Toolbar sx={{position:"absolute", top:0, zIndex:1000, backgroundColor:"black", borderBottom:3, borderColor:'divider', width:'100%', py:2}}>
        <Box
        component="img"
        sx={{
          mr:3,
      ...
      />
       <Stack direction="row" spacing={2} >
          <Button onClick={()=> scrollIntoView(home)}>
            Home
          </Button>
          <Button onClick={()=> scrollIntoView(about)}>
            About
          </Button>
          <Button onClick={()=> scrollIntoView(songs)}>
            Songs
          </Button>
          <Button onClick={()=> scrollIntoView(contact)}>
            Contact
          </Button>
          <Button onClick={()=> scrollIntoView(tour)}>
            Tour Dates
          </Button>
          <ScrollToTop />
        </Stack>
